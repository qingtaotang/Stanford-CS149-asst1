...
void computeAssignments(WorkerArgs *const args)
{
  ...
  std::cout <<"time of thread"<<args->threadId<<":"<< end-start << std::endl;
}

void computeAssignmentsThread(WorkerArgs *const argsAll)
{
   int numThreads = 16;
  ...
  for (int i = 0; i < numThreads; i++)
  {
    ...
    args[i].data = argsAll->data;
    args[i].clusterCentroids = argsAll->clusterCentroids;
    args[i].clusterAssignments =argsAll->clusterAssignments;
    args[i].currCost = argsAll->currCost;
    args[i].M =argsAll-> M;
    args[i].N =argsAll-> N;
    args[i].K =argsAll-> K;
    args[i].start = argsAll-> M/numThreads*i;
    args[i].end = argsAll-> M/numThreads*(i+1);
    args[i].threadId = i;
  }
                  ...
                  int M, int N, int K, double epsilon)
{
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K))
  {
    ...
    double assignmentCost = t2 - t1;

    computeCentroids(&args);
    double t3 = CycleTimer::currentSeconds();
    double centroids = t3 - t2;

    computeCost(&args);
    double t4 = CycleTimer::currentSeconds();
    double computeCost = t4 - t3;

    cout << "Iteration " << iter << " timings:" << endl;
    cout << "Assignment cost: " << assignmentCost << " seconds" << endl;
    cout << "Centroid computation: " << centroids << " seconds" << endl;
    cout << "Cost computation: " << computeCost << " seconds" << endl;
