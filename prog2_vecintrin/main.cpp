...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
    ...
    __cs149_vec_float const99 = _cs149_vset_float(9.999999f);
    __cs149_mask maskAll, maskIsZero, maskIsNotZero, maskGreaterThan99;
    ...
    for (int i=0; i<N; i+=VECTOR_WIDTH) {
        ...
        while(_cs149_cntbits(maskIsNotZero)){
            ...
            _cs149_vsub_int(y,y,oneInt,maskIsNotZero);
            _cs149_veq_int(maskIsZero,y,zero,maskAll);
            maskIsNotZero = _cs149_mask_not(maskIsZero);
        }
        _cs149_vgt_float(maskGreaterThan99,result,const99,maskAll);
        _cs149_vset_float(result,9.999999f,maskGreaterThan99);
        // Write results back to memory
        _cs149_vstore_float(output+i, result, maskAll);
    }
